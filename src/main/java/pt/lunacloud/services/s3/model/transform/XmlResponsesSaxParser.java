 ...
 */
package pt.lunacloud.services.s3.model.transform;
...
import pt.lunacloud.services.s3.internal.ObjectExpirationResult;
import pt.lunacloud.services.s3.internal.ServerSideEncryptionResult;
...
import pt.lunacloud.services.s3.model.BucketLifecycleConfiguration.Rule;
import pt.lunacloud.services.s3.model.BucketLifecycleConfiguration.Transition;
...
import pt.lunacloud.services.s3.model.BucketNotificationConfiguration.TopicConfiguration;
...
import pt.lunacloud.services.s3.model.CORSRule;
import pt.lunacloud.services.s3.model.CORSRule.AllowedMethods;
import pt.lunacloud.services.s3.model.CanonicalGrantee;
...
import pt.lunacloud.services.s3.model.DeleteObjectsResult.DeletedObject;
import pt.lunacloud.services.s3.model.EmailAddressGrantee;
...
import pt.lunacloud.services.s3.model.MultiObjectDeleteException.DeleteError;
import pt.lunacloud.services.s3.model.MultipartUpload;
...
import pt.lunacloud.services.s3.model.PartSummary;
import pt.lunacloud.services.s3.model.Permission;
import pt.lunacloud.services.s3.model.RedirectRule;
import pt.lunacloud.services.s3.model.RoutingRule;
import pt.lunacloud.services.s3.model.RoutingRuleCondition;
import pt.lunacloud.services.s3.model.S3Object;
import pt.lunacloud.services.s3.model.S3ObjectSummary;
import pt.lunacloud.services.s3.model.S3VersionSummary;
import pt.lunacloud.services.s3.model.StorageClass;
import pt.lunacloud.services.s3.model.TagSet;
...
public class XmlResponsesSaxParser {
	private static final Log log = LogFactory
	        .getLog(XmlResponsesSaxParser.class);
	...
	protected InputStream sanitizeXmlDocument(DefaultHandler handler,
		...
		if (!sanitizeXmlDocument) {
			...
			if (log.isDebugEnabled()) {
				log.debug("Sanitizing XML document destined for handler "
				        + handler.getClass());
			}
			...
			try {
				 ...
				 * parse the XML.
				 */
				String listingDoc = listingDocBuffer.toString().replaceAll(
				        "\r", "&#013;");
	...
	public DeleteObjectsHandler parseDeletedObjectsResult(
	        InputStream inputStream) {
	...
	public class ListBucketHandler extends DefaultHandler {
		...
		public ObjectListing getObjectListing() {
			...
			if (nextMarker != null) {
				...
				if (objectListing.getObjectSummaries().isEmpty() == false) {
					...
					nextMarker = objectListing.getCommonPrefixes().get(
					        objectListing.getCommonPrefixes().size() - 1);
				} else {
		...
		public void endElement(String uri, String name, String qName) {
			...
			if (name.equals("Name")) {
				...
				String isTruncatedStr = elementText.toLowerCase(Locale
				        .getDefault());
			...
			// Object details.
				...
				try {
					currentObject.setLastModified(ServiceUtils
					        .parseIso8601Date(elementText));
	...
	public class ListAllMyBucketsHandler extends DefaultHandler {
		...
		public void endElement(String uri, String name, String qName) {
			...
			// Bucket item details.
				...
				elementText += ".000Z";
				try {
					currentBucket.setCreationDate(ServiceUtils
					        .parseIso8601Date(elementText));
				} catch (ParseException e) {
					throw new RuntimeException(
	...
	public class AccessControlListHandler extends DefaultHandler {
		...
		public void startElement(String uri, String name, String qName,
			...
			if (name.equals("Owner")) {
				...
				String type = XmlResponsesSaxParser.findAttributeValue(
				        "xsi:type", attrs);
		...
		public void endElement(String uri, String name, String qName) {
			...
			// ACL details.
				...
				accessControlList.grantPermission(currentGrantee,
				        currentPermission);
				currentGrantee = null;
		...
		public void endElement(String uri, String name, String qName) {
			...
			if (name.equals("TargetBucket")) {
				...
				bucketLoggingConfiguration
				        .setDestinationBucketName(targetBucket);
	...
	public class CopyObjectResultHandler extends DefaultHandler implements
	        ServerSideEncryptionResult, ObjectExpirationResult {
	...
	public class ListVersionsHandler extends DefaultHandler {
		...
		public void endElement(String uri, String name, String qName)
			...
			if (name.equals("ListVersionsResult")) {
				...
				versionListing.getCommonPrefixes().add(
				        checkForEmptyString(text.toString()));
			} else if (name.equals("CommonPrefixes")) {
				insideCommonPrefixes = false;
			} else if (name.equals("KeyMarker")) {
				versionListing
				        .setKeyMarker(checkForEmptyString(text.toString()));
			} else if (name.equals("VersionIdMarker")) {
				versionListing.setVersionIdMarker(checkForEmptyString(text
				        .toString()));
			} else if (name.equals("MaxKeys")) {
				versionListing.setMaxKeys(Integer.parseInt(text.toString()));
			} else if (name.equals("Delimiter")) {
				versionListing
				        .setDelimiter(checkForEmptyString(text.toString()));
			} else if (name.equals("NextKeyMarker")) {
				...
				currentVersionSummary
				        .setIsLatest("true".equals(text.toString()));
			} else if (name.equals("LastModified")) {
				assert (currentVersionSummary != null);
				try {
					currentVersionSummary.setLastModified(ServiceUtils
					        .parseIso8601Date(text.toString()));
				} catch (ParseException e) {
				...
				currentVersionSummary.setETag(ServiceUtils.removeQuotes(text
				        .toString()));
			} else if (name.equals("Size")) {
				assert (currentVersionSummary != null);
				assert (!currentVersionSummary.isDeleteMarker());
	...
	public class BucketWebsiteConfigurationHandler extends DefaultHandler {
		private BucketWebsiteConfiguration configuration = new BucketWebsiteConfiguration(
		        null);
		...
		public void startElement(String uri, String name, String qName,
			...
			if (name.equals("WebsiteConfiguration")) {
				...
				inRedirect = true;
			        ...
			        && (inRedirect || inRedirectAllRequestsTo)) {
	...
	public class CompleteMultipartUploadHandler extends DefaultHandler
	        implements ServerSideEncryptionResult, ObjectExpirationResult {
		...
		/**
		 * @see pt.lunacloud.services.s3.model.CompleteMultipartUploadResult#getExpirationTime()
		 */
		public Date getExpirationTime() {
			if (result != null)
		...
		/**
		 * @see pt.lunacloud.services.s3.model.CompleteMultipartUploadResult#setExpirationTime(java.util.Date)
		 */
		public void setExpirationTime(Date expirationTime) {
			if (result != null)
		...
		/**
		 * @see pt.lunacloud.services.s3.model.CompleteMultipartUploadResult#getExpirationTimeRuleId()
		 */
		public String getExpirationTimeRuleId() {
			if (result != null)
		...
		/**
		 * @see pt.lunacloud.services.s3.model.CompleteMultipartUploadResult#setExpirationTimeRuleId(java.lang.String)
		 */
		public void setExpirationTimeRuleId(String expirationTimeRuleId) {
			if (result != null)
	...
	public class ListMultipartUploadsHandler extends DefaultHandler {
		...
		public void endElement(String uri, String name, String qName)
			...
			if (name.equals("ListMultipartUploadsResult")) {
				...
				result.setNextUploadIdMarker(checkForEmptyString(text
				        .toString()));
			} else if (name.equals("MaxUploads")) {
				...
				currentOwner
				        .setDisplayName(checkForEmptyString(text.toString()));
				...
				currentInitiator.setDisplayName(checkForEmptyString(text
				        .toString()));
			} else if (name.equals("StorageClass")) {
				currentMultipartUpload.setStorageClass(text.toString());
			} else if (name.equals("Initiated")) {
				try {
					currentMultipartUpload.setInitiated(ServiceUtils
					        .parseIso8601Date(text.toString()));
				} catch (ParseException e) {
					throw new SAXException(
	...
	public class ListPartsHandler extends DefaultHandler {
		...
		public void endElement(String uri, String name, String qName)
			...
			if (name.equals("ListPartsResult")) {
				...
				try {
					currentPart.setLastModified(ServiceUtils
					        .parseIso8601Date(text.toString()));
				} catch (ParseException e) {
					throw new SAXException(
	...
	public class BucketNotificationConfigurationHandler extends DefaultHandler {
		...
		public void startElement(String uri, String name, String qName,
			...
			if (name.equals("NotificationConfiguration")) {
				topicConfigurations = new ArrayList<BucketNotificationConfiguration.TopicConfiguration>(
				        1);
	 ...
	 * Connection: keep-alive Server: AmazonS3
	 * 
	...
	public class DeleteObjectsHandler extends DefaultHandler {
		...
		public void endElement(String uri, String name, String qName)
			...
			if (name.equals("Deleted")) {
				...
				if (deletedObject != null) {
					deletedObject.setDeleteMarker(text.toString()
					        .equals("true"));
				}
			} else if (name.equals("DeleteMarkerVersionId")) {
				if (deletedObject != null) {
	 ...
	 * <Days>365</Days> </Expiration> </Rule> <Rule> <ID>image-rule</ID>
	 * <Prefix>image/</Prefix> <Status>Enabled</Status> <Transition>
	...
	public class BucketLifecycleConfigurationHandler extends DefaultHandler {
		...
		public void endElement(String uri, String name, String qName)
			...
			if (name.equals("LifecycleConfiguration")) {
				...
				transition.setStorageClass(StorageClass.fromValue(text
				        .toString()));
			} else if (name.equals("Date")) {
				if (inTransition == false) {
					try {
						rule.setExpirationDate(ServiceUtils
						        .parseIso8601Date(text.toString()));
					} catch (ParseException e) {
					...
					try {
						transition.setDate(ServiceUtils.parseIso8601Date(text
						        .toString()));
					} catch (ParseException e) {
	...
	private static String findAttributeValue(String qnameToFind,
	        Attributes attrs) {
